...
Only `autism-ap-builder.html`, `README.md`, `CLAUDE.md`, `docs/`, `tests/`, `package.json`, and the shared parts of `.claude/` (`skills/`, `commands/`, `agents/`, `third-party/`) are tracked in git and pushed to GitHub. Everything else is local. The `.claude/` pieces are tracked so cloud sessions, which start from a fresh clone, get /verify and /council. Cloud sessions do **not** install plugins, so /feature-dev (with its three agents) and /engineering-code-review are committed as copies; `.claude/third-party/README.md` lists their sources and license, and the local sparse-checkout command that hides them on a desktop where the real plugins are installed. Do not edit those copies except to refresh them from upstream.
