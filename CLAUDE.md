...
├── .claude/                     Partly tracked: `skills/` (/verify), `commands/` (/council), `settings.json` (plugins for cloud sessions);
...
Only `autism-ap-builder.html`, `README.md`, `CLAUDE.md`, `docs/`, `tests/`, `package.json`, and the shared parts of `.claude/` (skills, commands, `settings.json`) are tracked in git and pushed to GitHub. Everything else is local. The `.claude/` pieces are tracked so cloud sessions, which start from a fresh clone, get the same /verify, /council, /feature-dev and /engineering:code-review as the desktop.
...
**`{placeholder}` syntax:** Used in ABA and IEP letter output for fields the clinician fills in after copying. Rendered with yellow CSS highlighting (`.aba-ph`, `.iep-ph`) in the on-screen preview only. The Word (rich) copy and the Epic (plain) copy both ship the braces verbatim and unstyled: Word carries inline run formatting into whatever the clinician types over a placeholder, and Epic already treats `{...}` as an F2 cursor stop, so there is no `***` rewrite.
