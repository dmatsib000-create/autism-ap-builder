# §4 Functional Need Domains — clinical-design decision record (closed)

**Status:** **CLOSED 2026-06-12** — this review is complete; it is now a historical decision record, not active work. All seven ratified units shipped (Unit 1 D1/D2 ABA-sync fixes; Unit 2 motor split + hypotonia→PT; Unit 3 clarity relabels; Unit 4-C2 OT/PT co-management line; Unit 5 academic→psychoeducational-eval auto-recommendation; Unit 6 fecal-smearing feature; Unit 7 toddler scoping), and both deferrals were resolved by David (Unit 4-E2 near-permanently deferred; C4 toe-walking → no dedicated item, covered by the existing focal-finding → neurology path). See "Resolved deferrals" below. · **Reviewed:** 2026-06-02 · **Scope:** the §4 Functional Need Domains section of [`autism-ap-builder.html`](../autism-ap-builder.html) — validity/completeness of the input items in each domain, and the output text those inputs drive (needs-summary prose + therapy-rule + plan/letter text).

**How to read this now.** This is a closed, point-in-time **decision record** — the *why* behind the §4 work: a four-pass clinical-design council, the roadmap it ratified, the alternatives it rejected, and the changes it deliberately did **not** make. It is immutable history and carries **no update obligation** (contrast the living `branching-logic.md` and `references.md`, which CLAUDE.md requires kept in sync). **For current behavior, read `branching-logic.md`, not this doc.** Reach for this when you need the *rationale* behind a §4 decision — e.g. why a flagged academic concern routes to a psychoeducational evaluation but not to medical ABA; why there is no toe-walking item; why reciprocity/Theory-of-Mind are double-listed across the DSM-5 criteria and §4 — the answers and the reasoning are here. Source references use the house `file:line` convention (e.g., `autism-ap-builder.html:5952`); `§N` references point to `branching-logic.md`. Every roadmap unit below is marked ✅ done; the two non-builds are recorded under "Resolved deferrals."
...
| **7** ✅ | **Toddler scoping** — **DONE 2026-06-12** | Early Steps "under 34 months" → "under 36 months" + promptness caveat (1B); Part C→Part B transition sentence inside the Early Steps block (confirmed/suspected toddlers — David resolved the rule-out micro-point: Early Steps placement suffices); full Part B output suppression for toddlers (placement/IEP-doc/services band, accommodations, SLP/OT school clauses kept frequency-only, concurrent-billing note) with `syncSchoolSvcFromNeeds` early-returning on toddler; §"School & Educational Supports" form section hidden for toddlers with stale-state teardown + **dynamic visible-folio renumbering** (contiguous 1–8). Locked by the updated `suspected-toddler` golden + new `toddler-stale-school` fixture (stale Part B state may not render); folio/hide/teardown browser-verified. | Yes (1 golden updated + 1 new fixture) | §2.1 / §10.9 updated | done |

**Resolved deferrals (David, 2026-06-12):** **C4** (toe-walking / gait item) — **not adding a dedicated item.** Toe-walking read as a neurological sign is already captured by the existing "Focal neurological findings → neurology referral" control (`focalNeuroFindings` → `ruleNeurology`); the only uncaptured slice is benign idiopathic toe-walking (PT/gait), judged low-value. **E2** (one-line note distinguishing the reciprocity/perspective double-listing as criteria-evidence vs. treatment-target) — **near-permanently deferred**; revisit only if a clinician reports the double-listing is genuinely confusing in real use. **Keeps (ratified, no change):** B1 sensory model, D4 `boundaryViol` dual-home, E3 pragmatics stays in Social/Play, E4 `emotionalReg` placement.
...
- **C4: RESOLVED (David, 2026-06-12) — no dedicated item.** Toe-walking as a neurological sign routes through the existing `focalNeuroFindings` ("Focal neurological findings → neurology referral", `autism-ap-builder.html:1342`) → `ruleNeurology` (`:2123`); the remaining benign/idiopathic toe-walking slice is low-value and does not warrant its own §4 control. Reopen only if the idiopathic-PT case proves worth capturing.
...
Toddlers (under 3) are served under **IDEA Part C** (early intervention / Early Steps), not **Part B** school services / IEP. Three changes align the tool with that boundary. Clinical forks were resolved by David (Q1–Q3); the one design question (folio numbering) was confirmed by a code check (Q4). **IMPLEMENTED 2026-06-12** — the rule-out-toddler micro-point in item 2 was resolved by David at implementation: the transition sentence lives inside the Early Steps block (confirmed/suspected only).
...
- **C4 (toe-walking/gait) — resolved 2026-06-12.** David's call: no dedicated item; the neuro case is covered by `focalNeuroFindings` → neurology, and the idiopathic/PT slice is low-value. Reopen only if that slice proves worth capturing.
