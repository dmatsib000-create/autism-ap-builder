...
>
> **Last verified against commit:** `7dd8b95`
...
6. If it warrants IEP-letter accommodations, add an entry to the `accomBeh` table (§10.7) via the per-comorbid switch
...
| `academic` | boolean | [§9.2](#92-aba-target-population--syncabatargetsfromneeds), [§10.10](#1010-psychoeducational-evaluation-auto-recommendation) |
...
| `paeDetail` | string | [§7.9](#79-fasd-flags--prenatal-alcohol-exposure--fasd-features) |
| `paeEvidence` | Set | [§7.9](#79-fasd-flags--prenatal-alcohol-exposure--fasd-features) |
| `paeFrequency` / `paeQuantity` / `paeTiming` | string | [§7.9](#79-fasd-flags--prenatal-alcohol-exposure--fasd-features) |
| `prenatalAlcohol` | boolean | [§7.9](#79-fasd-flags--prenatal-alcohol-exposure--fasd-features) |
...
| `schoolSvc` | Set | [§10.6](#106-service-rationale-blocks) |
| `schoolSvcAuto` / `schoolSvcManualOff` | Set | [§10.9](#109-the-schoolsvcauto--schoolsvcmanualoff-tracking) |
| `seizureConcern` | boolean | [§7.2](#72-the-18-rules-at-a-glance) |
| `sleepStudy` | boolean | [§7.2](#72-the-18-rules-at-a-glance) |
| `specifiers` | Set | [§9.7](#97-specifier-display--intentional-omissions), [§10.8](#108-specifier-edge-cases) |
...
| `traumaIncludeInIEP` | boolean | [§6.4](#64-the-trauma-iep-gate), [§10.4](#104-trauma-opt-in-gate) |
...
| `syncSchoolSvcFromNeeds()` toddler gate | ~6150 | `ageGroup === 'toddler'` → early return | No Part B school services are auto-populated for toddlers (see [§10.10](#1010-psychoeducational-evaluation-auto-recommendation)) |
...
- Educational impact bullets (§10.5) may add ADHD-specific framing.
- `accomBeh` (§10.7) adds: *extended time, preferential seating, chunked assignments*.
...
| 13 | `ruleAudiology` | 1417 | Hearing screen fail, OR (toddler/preschool AND speech concern), OR either FASD flag (§7.9 — rationale adapts; always a single bullet) | `audiology` |
...
- **One collaborative-deference lead-in prints once**, via the `therapyIntro()` print-once helper, immediately before the **first** therapy section that renders. All seven therapy sections call it at their top (ABA, PCIT, psychotherapy, social skills, SLP, OT, PT), so the lead-in leads whichever renders first and never repeats, even when ABA and the allied therapies are absent and a behavioral/psychological section (e.g. PCIT-only) is all that fires. Its text affirms that the needs and services are *clinically established* (preserving the medical-necessity framing a payer reads) and then defers *assessment detail, treatment selection, and service intensity* to the treating therapist. Do not duplicate it per section, and do not let it read as "provisional / pending" (that is a denial hook). When adding a new therapy-recommendation section, call `therapyIntro()` at its top.
...
### 7.9 FASD flags — prenatal alcohol exposure & FASD features
...
3. **Differential prose (R6, council-ratified 2026-07-16, amended same day):**
   ...
   - **Confirmed + criteria gate** → the evidentiary sentence ("Notably, the documented presentation includes deficits in nonverbal communicative behaviors and restricted, repetitive behaviors beyond the sensory domain…") prints **only when `criteriaA` has `a2` AND `criteriaB` has at least one of `b1`/`b2`/`b3`**. The sentence asserts only what the note itself documents; when the gate fails (in practice: a documentation gap, since any complete B set includes a non-sensory criterion), it stays silent rather than handing a reviewer the weak spot of a thinly documented presentation. This is the **single ratified exception** to the no-criteria-aware-differential-prose rule; anything beyond it is deferred.

**Referral wiring.**

- **Genetics** — either flag joins `ruleGenetics`'s flag arm (FASD diagnosis requires excluding genetic phenocopies). The referral bullet gains a `gxParts` clause ("a clinical picture concerning for fetal alcohol spectrum disorder"), the flags join `hasAnyGxFlag` (so the lead sentence claims clinical findings honestly), and a dedicated continuation line carries the phenocopy-exclusion rationale with NIAAA attribution. No new superscript — the two existing genetics citations are untouched.
- **Audiology** — either flag joins `ruleAudiology`, which now exposes `{include, base, fasd}` so the single bullet adapts: FASD-only trigger → FASD-workup rationale; base trigger only → the pre-existing text unchanged (byte-stable non-FASD notes); both → base text plus "also serves the FASD workup; no separate referral is needed." One bullet always — dedup with `hearingScreenFail` is by construction.
- **Ophthalmology** — fires **only when BOTH flags are set**: a plain plan line for a dilated eye examination per FASD workup guidance. Deliberately NOT an override rule (no `OV_DEFS` entry, no `S.overrides` key) — the invariants lane's override-registry parity is untouched. It has its own term in the MEDICAL REFERRALS section-open gate.
- **The Florida Center for Early Childhood (FASD Clinic)** — either flag emits a FAMILY RESOURCES line (a family *self*-referral, hence not Medical Referrals): the only FASD-specific diagnostic clinic in Florida; telehealth statewide (confirmed by David 2026-07-16); thefloridacenter.org. Tone per §7.8: name the resource, defer the process.
...
| `academics` | `S.academic === true && isYoung()` — **toddler/preschool only** (school-age academics belong to IDEA/FAPE, not ABA). For school-age and up, the academic *concern* instead routes to the `psychoed` school eval — see [§10.10](#1010-psychoeducational-evaluation-auto-recommendation) |
...
which does not condition eligibility on any particular medical diagnosis
(34 C.F.R. §§300.8, 300.304(c)(6)).
...
|---|---|---|
| Language Impaired | `comorbid.has('language_disorder')`, any `needsComm` expressive/receptive/AAC, or `langLevel` in nonverbal/singleWord/phrase | §300.8(c)(11) |
| Speech Impaired | `needsComm.has('oral_motor')`, `hasSpeechSoundErrors()`, `hasReducedIntell()` | §300.8(c)(11) |
| Other Health Impaired | `hasConfirmedADHD()`, `adhd_suspected`, `epilepsy`, `sleep_disorder` | §300.8(c)(9) |
...
| Intellectual Disability | `hasID()` or `specifiers.has('withID')` | §300.8(c)(6) |
| Developmental Delay | `ageGroup === 'preschool'`, `withGDD`, or `withSuspectedGDD` | §300.8(b) |

Speech Impaired and Language Impaired are listed separately on purpose: different evaluators,
different criteria, and a student can qualify for one and not the other.

**Emotional/Behavioral Disability is deliberately not auto-derived.** Districts weight physician
language heavily; an EBD determination carries placement and disciplinary consequences; anxiety
and depression in this population are frequently secondary to the neurodevelopmental profile;
and the tool captures none of the chronicity and severity data the category requires. A clinician
who wants it writes it into the fallback bracket. Autism is never listed — the letter just ruled
it out.
...
|---|---|---|
| `ruleOutEvalRequest` | `schoolDoc` is `iep_needed` or `neither` | The "diagnosis pending" paragraph; anchors on child find (34 C.F.R. §§300.111, 300.301) |
...
### 10.5 Educational impact bullets
...
### 10.9 The `schoolSvcAuto` / `schoolSvcManualOff` tracking
...
### 10.10 Psychoeducational-evaluation auto-recommendation
...
The single `schoolSvc.psychoed` entry then drives the recommendation in both the note (School / Educational Supports block) and the IEP letter (eval-request paragraph + the §10.6 `psychoed` service block) with no per-surface duplication.
...
**Source variants.** The **specifier line** has four output variants, all routed through `applySpecLabelTransforms()` → `withBifSourceQualifier` (§11.12). The **clinical summary** now carries a parallel four-branch qualifier of its own (`bifSummaryClause()`) — it was previously source-blind, asserting "in the borderline intellectual functioning (BIF) range" identically for a KBIT-2R and a WISC-V, and it renders whether or not `withBIF` is checked. Both four-branch maps must be updated together if a fifth `cogDataSource` value is ever added: bare for `comprehensive` and `priorExternal` (priorExternal additionally appends a "per prior outside evaluation; supporting records not available for direct review" attribution — see §11.15), a "brief screening only" qualifier for `screener`, and a "clinical impression only" qualifier for `clinical`. No path produces bare BIF with no source on file.

**The ID-risk state (`bifIdRisk()`, added 2026-07-20).** A borderline call resting on a **screener** or a **clinical impression** has not excluded intellectual disability: comprehensive testing may land in the ID range, and even a confirmed borderline score can support ID when a standardized adaptive measure is impaired (DSM-5-TR weights adaptive functioning and reads the 70-75 band with its measurement error). `bifIdRisk()` returns true for exactly those two sources and drives three things — the clinical-summary clause (`bifSummaryClause()`), a follow-on caveat sentence (`bifIdRiskSentence()`), and the confirmatory cognitive/adaptive evaluation referral (its own paragraph in Diagnostic Workup, reached via `hasCogEvalReferral`, which also opens the section and can retitle it `COGNITIVE / ADAPTIVE EVALUATION`).

It is **derived only** and does not disturb the asymmetries above: it sets no specifier, is not a bridge target, and `currentClinicalPathway()` still returns `specKey:null` for borderline. It is deliberately *not* modelled as `withSuspectedBIF` or as `withSuspectedID` — the note says ID is *not yet excluded*, which is weaker and more accurate than *suspected*. The `comprehensive` and `priorExternal` sources are excluded (nothing left to confirm; prior-external has its own attribution path per §11.15). Locked by `bif-screener-id-risk` (fires) and `bif-comprehensive-established` (must not fire) in the golden lane, plus seven `bifIdRisk` unit cases including a derived-only invariant assertion.
...
- `withSuspectedGDD` → **developmental** tier: a single required component, "comprehensive standardized developmental assessment (e.g., the Bayley-4, the Battelle Developmental Inventory 3rd ed., the Mullen Scales of Early Learning, or Griffiths III)", with the ASQ-3 / SWYC named as the insufficient screeners, and eligibility phrased for GDD alone. **No adaptive measure is required or mentioned** — this matches `confirmableGdd` above (a comprehensive developmental battery alone is sufficient; adaptive is informative but not criterial per DSM-5-TR/AACAP, and instruments like the Bayley-4 already carry adaptive subscales). The requirement, not just the instrument list, is tier-specific: the pre-2026-07-20 shared paragraph asserted "both components" for GDD as well, contradicting the app's own rule and reading to a receiving evaluator as blocking the diagnosis pending a test that is not actually required (David, 2026-07-20).
